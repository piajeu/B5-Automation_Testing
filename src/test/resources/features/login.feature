Feature: FR01 - Login
  ...
  Scenario: TC02 - Pelajar gagal login karena kata sandi tidak sesuai
    Given aplikasi JTK Learn terbuka di halaman login "https://polban-space.cloudias79.com/jtk-learn/"
    When pelajar memasukkan email "devi123@example.com"
    And pelajar memasukkan password "wrongpass123"
    And pelajar mengklik tombol Masuk
    Then sistem memvalidasi kredensial, login gagal, dan halaman tetap di halaman Login
    And sistem menampilkan pesan error login "Password yang dimasukkan tidak sesuai!"